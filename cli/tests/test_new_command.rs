...
fn test_new_insert_after() {
    ...
    Parent commit      : kkmpptxz 6041917c (empty) B |B
    Parent commit      : vruxwmqv c9257eff (empty) D |D
    "###);
    insta::assert_snapshot!(get_short_log_output(&test_env, &repo_path), @r###"
    ◉  C
    │ ◉  F
...
fn test_new_insert_after_children() {
    ...
    Parent commit      : qpvuntsm 65b1ef43 (empty) A |A
    Parent commit      : mzvwutvl ec18c57d (empty) C |C
    "###);
    insta::assert_snapshot!(get_short_log_output(&test_env, &repo_path), @r###"
    @    G
    ├─╮
...
fn test_new_insert_before() {
    ...
    Parent commit      : znkkpsqq 41a89ffc (empty) E |E
    Parent commit      : vruxwmqv c9257eff (empty) D |D
    Parent commit      : kkmpptxz 6041917c (empty) B |B
    "###);
    insta::assert_snapshot!(get_short_log_output(&test_env, &repo_path), @r###"
